...
The database is migrated and seeded automatically before `dev`/`build` (via the `predev`/`prebuild` npm scripts) and is written to the gitignored `tailspin.db` file. The home page also includes category and publisher filters so backers can narrow the game list without leaving the catalog page.
